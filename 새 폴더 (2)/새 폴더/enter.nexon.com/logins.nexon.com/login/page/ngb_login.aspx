...
var NgbSecurity = new function __NgbSecurity()
{
	...
	var _timeout;
	var _header;
	var _LTTLGBNA4WRM1JHG = new Array();
	...
	this.InitData = function()
	{
		...
		_LTTLGBNA4WRM1JHG.length = 0;
	}
	
	this.AddData = function( _JYOFTOXI98M9ODXM )
	{
		if ( typeof( _url ) != 'undefined' && typeof( _timer ) != 'undefined' )
			return;
			
		_LTTLGBNA4WRM1JHG[ _LTTLGBNA4WRM1JHG.length ] = _JYOFTOXI98M9ODXM;
	}
	...
	{
		if ( _LTTLGBNA4WRM1JHG.length == 0 || typeof( handler ) == 'undefined' )
			throw Exception;
	...
	this.HashString = function( _WTJME8838DE8H8YQ )
	{
		var hashString = NgbHash.HMAC_SHA256_MAC2( "4E65786F6E55736572", _WTJME8838DE8H8YQ );
		return hashString;
	}
	
	this.HashString2 = function( _2FVJ41JSLRXXKIS0, _WTJME8838DE8H8YQ, _MO6VJAVY3WDE42EU )
	{
		var hashString = _2FVJ41JSLRXXKIS0 + NgbHash.HMAC_SHA256_MAC( _MO6VJAVY3WDE42EU, NgbHash.HMAC_SHA256_MAC2( "4E65786F6E55736572", _WTJME8838DE8H8YQ ));
		return hashString;
	}
	...
	var ProcessEncrypt = function()
	{
		...
		var _JYOFTOXI98M9ODXM = '';
		
		if ( typeof( _hash ) != 'undefined' )
			_JYOFTOXI98M9ODXM = _hash + '\\';
			
		for ( var i = 0; i < _LTTLGBNA4WRM1JHG.length; i++ )
		{
			_JYOFTOXI98M9ODXM += NgbRSA.utf8base64encode( _LTTLGBNA4WRM1JHG[ i ] );
			
			if ( i < _LTTLGBNA4WRM1JHG.length - 1 )
				_JYOFTOXI98M9ODXM += '\\';
		}
		
		var val = EncryptString( key, _JYOFTOXI98M9ODXM );
		_LTTLGBNA4WRM1JHG.length = 0;
	...
	var EncryptString = function( key, _YN6338F0OPB2N11H )
	{
		...
		var a = new Array();
		var sl = _YN6338F0OPB2N11H.length;
		
		var i = 0;
		while ( i < sl )
		{
			a[ i ] = _YN6338F0OPB2N11H.charCodeAt( i );
			i++;
		}
...
var NgbLogin = new function __NgbLogin()
{
	...
	var _header;
	var _hashKey;
	var __LUWIQ51OU84G7D80;
	var __DXMQYDMI98M1UDXI;
	var _isLogin2;
	var _callBackMethod;
	var _isKeepLogin = '0';
	...
	var _P7NX88HML3M8SLM7;
	
	var _isInvalidPasswordHashKeyString = false;
    var _isFailover = false;
    var _isSLogin = '0';
	
	this.SetData = function( _LUWIQ51OU84G7D80, _DXMQYDMI98M1UDXI, accesscode, strRedirect, isSLogin )
	{
		__LUWIQ51OU84G7D80 = _LUWIQ51OU84G7D80;
		__DXMQYDMI98M1UDXI = _DXMQYDMI98M1UDXI;
    ...
    this.SLogin = function (_LUWIQ51OU84G7D80, _DXMQYDMI98M1UDXI, accesscode, strRedirect) {
        NgbLogin.SetData(_LUWIQ51OU84G7D80, _DXMQYDMI98M1UDXI, accesscode, strRedirect, '1');
        _isLogin2 = false;

        if (_LUWIQ51OU84G7D80.indexOf('@') != -1 )
        {
            var arrEmail = _LUWIQ51OU84G7D80.split('@');

            if (arrEmail.length != 2 || arrEmail[0] == '' || arrEmail[1] == '' || _LUWIQ51OU84G7D80.indexOf(' ') != -1 )
            {
                alert('아이디를 확인해 주세요.');
                return;
            }
            ...
            _isFailover = false;
            _P7NX88HML3M8SLM7 = _LUWIQ51OU84G7D80;
            AuthSystem.GetPasswordHashKey(_LUWIQ51OU84G7D80, NgbLogin.HashHandler);
    ...
    this.Login = function (_LUWIQ51OU84G7D80, _DXMQYDMI98M1UDXI, accesscode, strRedirect)
	{
		NgbLogin.SetData( _LUWIQ51OU84G7D80, _DXMQYDMI98M1UDXI, accesscode, strRedirect);
		_isLogin2 = false;
	...
	this.Login2 = function( _LUWIQ51OU84G7D80, _DXMQYDMI98M1UDXI, callBackMethod, accesscode )
	{
        ...
        NgbLogin.SetData(_LUWIQ51OU84G7D80, _DXMQYDMI98M1UDXI, accesscode);
		_isLogin2 = true;
	...
	{
		_LUWIQ51OU84G7D80 = __LUWIQ51OU84G7D80;
		_DXMQYDMI98M1UDXI = __DXMQYDMI98M1UDXI;
		accesscode = _accesscode;
		strRedirect = _strRedirect;
        isSLogin = _isSLogin;
		
		var strDomain = NgbLogin.GetLoginURL();
		_LUWIQ51OU84G7D80 = NgbString.Trim( _LUWIQ51OU84G7D80 );
		_DXMQYDMI98M1UDXI = NgbString.Trim( _DXMQYDMI98M1UDXI );
		...
		if ( _LUWIQ51OU84G7D80 == '' || _LUWIQ51OU84G7D80.indexOf( ' ' ) != -1 )
		{
			alert( '아이디를 입력해 주세요.' );
			return;
		}
		else if ( _DXMQYDMI98M1UDXI == '' )
		{
			alert( '비밀번호를 입력해 주세요.' );
			return;
		}
		...
		try
		{
			...
			NgbSecurity.AddData( _LUWIQ51OU84G7D80 );
			NgbSecurity.AddData( _LUWIQ51OU84G7D80.indexOf( '@' ) != -1 ? NgbSecurity.HashString2( _header, _DXMQYDMI98M1UDXI, _hashKey ) : _DXMQYDMI98M1UDXI );
            NgbSecurity.AddData( NgbHash.SHA256_hash(_DXMQYDMI98M1UDXI));
