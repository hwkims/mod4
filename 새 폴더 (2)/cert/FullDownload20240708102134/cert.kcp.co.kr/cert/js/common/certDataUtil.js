...
function fn_validation_sms( deviceType )
{
    ...
    {
        alert( "인증자 이름을 입력해 주세요." , function(){ document.form_auth.user_name.focus(); });
    }
    else if( c_hangul( document.form_auth.user_name )  && ( PRE_MASK_YN != "Y" ) )
    {
        alert( "인증자 이름은 한글 또는 영문으로 입력해 주세요." , function(){ document.form_auth.user_name.focus(); });
    }
    ...
    {
        alert( "이름에 연속된 숫자 입력은 불가합니다." , function(){ document.form_auth.user_name.focus(); });
    }
    else
    {
        // validation 검증 완료
...
function fn_validation_otp()
{
    ...
    {
        alert("인증번호를 입력 해 주시기 바랍니다.", function(){ document.form_auth.otp_no.focus(); });
    }
...
function fn_validation_value_no_alert(){
    ...
    if( document.form_auth.user_name.value == "" ) {
    } else if( c_hangul( document.form_auth.user_name )       ) {
    } else if( document.form_auth.mynum1.value == ""        ||
               document.form_auth.mynum1.value.length != 6  ||
               !birthDayCheck(document.form_auth.mynum1.value) ) {
    } else if( document.form_auth.mynum2.value == ""        ||
        document.form_auth.mynum2.value.length != 1    ) {
    } else if(!document.form_auth.phone_no.value.isMobile()   ) {
    } else {
        isValid = true;
    }
